...
const std::string Level2::playID = "LEVEL_2";
...
void Level2::render() {
    ...
    TheMapParser::Instance()->getMaps("level2")->render();
...
void Level2::update() {
    ...
    if (m_winning || pPlayer->getHealth() == 0) {
        onExit();
    ...
    if (pPlayer->getPosition().getX() >= 1150 && pPlayer->getHealth() > 0 && TheMapParser::Instance()->getTileLayers("level2")[0]->getPosition().getX() == TILE_LAYER_WIDTH - GAME_WIDTH) {
    ...
    if (TheMapParser::Instance()->getTileLayers("level2")[0]->getPosition().getX() == TILE_LAYER_WIDTH - GAME_WIDTH) {
        for (auto i : *TheMapParser::Instance()->getObjectLayer("level2")[0]->getGameObjects()) {
            if (i->getID() == "boss" || i->getID() == "cyrer") {
                i->setVel(0, 0);
            }
        }
    }
    ...
    TheMapParser::Instance()->getMaps("level2")->update();

    checkBullet_TileCollision(TheMapParser::Instance()->getTileLayers("level2"));
    checkEnemy_PlayerBulletCollision(m_gameObjects);
    checkEnemy_PlayerBulletCollision(*TheMapParser::Instance()->getObjectLayer("level2")[0]->getGameObjects());
    checkPlayer_EnemyBulletCollision(pPlayer);
    checkPlayer_EnemyCollision(pPlayer, *TheMapParser::Instance()->getObjectLayer("level2")[0]->getGameObjects());
    checkPlayer_TileCollision(pPlayer, TheMapParser::Instance()->getTileLayers("level2"));
...
bool Level2::onEnter() {
    ...
    if (!TheTextureManager::Instance()->loadTexture("res/boss.png", "boss", TheGame::Instance()->getRenderer()))
    {
        return false;
    }
    ...
    if (!TheMapParser::Instance()->parse("level2", "res/Tile/level2.tmx")) {
        return false;
    }
...
bool Level2::onExit() {
    ...
    std::cout << "Exiting level 2" << std::endl;
    return true;
}

//Checking collision
void Level2::checkPlayer_EnemyBulletCollision(Player* pPlayer)
{
...
void Level2::checkEnemy_PlayerBulletCollision(const std::vector<GameObject*>& objects)
{
...
void Level2::checkPlayer_EnemyCollision(Player* pPlayer, const std::vector<GameObject*>& objects)
{
    if (pPlayer->getWinningDrawing()) {
        return;
    }
...
void Level2::checkPlayer_TileCollision(Player* pPlayer, const std::vector<TileLayer*>& collisionLayers)
{
    if (pPlayer->getWinningDrawing()) {
        return;
    }
...
void Level2::checkBullet_TileCollision(const std::vector<TileLayer*>& collisionLayers) {
    for (std::vector<TileLayer*>::const_iterator it = collisionLayers.begin(); it != collisionLayers.end(); it++)
    {
        TileLayer* pTileLayer = (*it);
        std::vector<std::vector<int>> tiles = pTileLayer->getTileMap();
